<template>
    <div class="installation-table-container">
        ...
        >
                ...
                <template #default="{ row: projectRow }">
                    <div style="padding: 10px 20px; background-color: #f5f7fa;">
                        <h4 style="margin-top: 0; color: #606266">
                            Danh sách trạng thái của thiết bị thuộc dự án - {{ projectRow.project_code }}
                        </h4>

                        <el-table :data="getGroupedStatusByProject(projectRow.items)" border stripe size="small">
            ...
            <el-table-column prop="project_code" :label="langStore.t('projectCodeLabel')" sortable />
        ...
        <el-dialog
            ...
            :title="`Chi tiết hàng hóa dự án: ${selectedProjectCode} - ${getStatusName(selectedStatus)}`"
            width="80%"
            align-center
            append-to-body
        ...
        >
                    ...
                    <el-table-column type="expand">
                        <template #default="{ row: partNoRow }">
                            <div style="padding: 10px;">
                                <el-table :data="partNoRow.items" border size="small">
                                    ...
                                    <el-table-column :label="langStore.t('tableHeaderAction')" width="100" fixed="right">
                                        <template #default="{ row }">
                                            <el-button
                                                type="success"
                                                circle
            ...
            <template #footer>
                <span class="dialog-footer">
                    <el-button @click="dialogPartNoVisible = false">{{ langStore.t('closeButton') }}</el-button>
                </span>
            </template>
        </el-dialog>
    </div>
    ...
    setup() {
        ...
        const groupedByProject = computed(() => {
            ...
            filteredInstallationItems.value.forEach(item => {
                const pCode = item.project_code || 'No Project';
                if (!groups[pCode]) {
        ...
        const groupedPartNosInPopup = computed(() => {
            if (!selectedStatusItems.value.length) return [];
            return groupItemsByPartNo(selectedStatusItems.value);
        });
        ...
        const getStatusType = (val) => (val === 0 ? 'success' : val === 1 ? 'warning' : 'info');
